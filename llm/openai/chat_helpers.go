...
func (chat *Chat) processFiles(ctx context.Context, fullMsg db.Message) (int, error) {
    ...
    for _, file := range fullMsg.Files {
        ...
        key, err := chat.ensureExposedFile(&file)
        if err != nil {
            chat.deps.Logger.With(logger.ERROR, err).Error("ensure exposed file failed")
            continue
        }

        fileURL := fmt.Sprintf("%s/%s", config.FileBucket(), key)
        chat.deps.Logger.Debug("requesting extraction for file", "url", fileURL)

        extracted, err := chat.extractFromFile(ctx, fileURL)
        if err != nil {
            chat.deps.Logger.With(logger.ERROR, err).Error("extraction failed")
            continue
        }

        created, err := chat.persistExtraction(file, extracted)
        if err != nil {
            chat.deps.Logger.With(logger.ERROR, err).Error("persisting extraction failed")
            continue
        }
...
func (chat *Chat) ensureExposedFile(file *db.File) (string, error) {
    if file.ExposedFile != nil && file.ExposedFile.Key != "" {
        return file.ExposedFile.Key, nil
    }
...
// extractFromFile asks OpenAI to extract structured JSON from the file URL
func (chat *Chat) extractFromFile(ctx context.Context, fileURL string) (extractionResult, error) {
    ...
    params := openai.ChatCompletionNewParams{
        ...
        Messages: []openai.ChatCompletionMessageParamUnion{
            ...
            openai.UserMessage(openai.TextContentPart(prompt)),
        },
    }
...
func (chat *Chat) persistExtraction(file db.File, res extractionResult) (int, error) {
    created := 0
